...
public interface OrderMapper {
    ...
    @Insert({"INSERT INTO order_table (uuid,zj,address,phone,ddbh,cjtime,account,busid,type) VALUES (#{model.uuid},#{model.zj},#{model.address}," +
            "#{model.phone},#{model.ddbh},#{model.cjTime},#{model.account},#{model.busid},-1)"})
    int addOrder(@Param("model") OrderModel model);

    @Insert({"INSERT INTO ordersp_table VALUES (#{model.uuid},#{model.spid},#{model.spsl}," +
            "#{model.spdj},#{model.spzj},#{model.orderid},0)"})
